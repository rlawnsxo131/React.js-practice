...
module.exports = {
  mode: 'production',
  entry: paths.ssrJs,
  target: 'node', // node 전용으로 번들링한다는 것을 명시한다.
  output: {
    path: paths.ssrBuild,
    filename: 'render.js',
    ...
    rules: [
      {
        ...
        // 최하단의 file-loader를 실행시킨다
        oneOf: [
          {
            // 자바스크립트 파일은 바벨을 사용하여 변환한다
            test: /\.(js|jsx)$/,
          ...
          css-loaders/locals를 실행하는 것이 중요하다. 
          파일을 따로 만들어내지 않기 때문 */
          {
            test: /\.css$/,
          ...
          // 만약에 자바스크립트도, 스타일도 아니라면 파일로 취급한다.
          // 여기서 emitFile: false 설정이 중요하다.
          {
